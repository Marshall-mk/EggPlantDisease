...
from src.cnn_models import dense_net_model, vgg16_model, efficient_net_model, compile_model
from src.data import DataLoader
import tensorflow as tf

EXPERIMENT_NAME = "Eggplant Disease Classification"
EXPERIMENT_ID = mlflow.create_experiment(EXPERIMENT_NAME)
MLFLOW_TRACKING_URI="https://dagshub.com/Marshall-mk/x.mlflow"
...
def main(cfg):
    ...
    # Load the model
    model = dense_net_model(input_shape=(cfg.model.image_size, cfg.model.image_size, 3), classes=cfg.model.classes)
    ...
    earlystopping = tf.keras.callbacks.EarlyStopping(monitor='val_loss',
                                                    ...
                                                    verbose=1, 
                                                    patience=5)
    checkpointer = tf.keras.callbacks.ModelCheckpoint(filepath=f'{cfg.model.ckpt_path}model.h5',
    ...
    report = {k: dict(sorted(v.items(), key=lambda item: item[1], reverse=True)) for k, v in report.items()}
    report = {k: {k2: v2 for k2, v2 in v.items() if k2 != "support"} for k, v in report.items()}
    report = {k: {k2: v2 for k2, v2 in v.items() if k2 != "macro avg"} for k, v in report.items()}
    report = {k: {k2: v2 for k2, v2 in v.items() if k2 != "weighted avg"} for k, v in report.items()}
